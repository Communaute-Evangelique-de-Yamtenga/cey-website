...
export async function GET() {
    try {
        ...
        const classifiedIds = new Set([...culteFB, ...louangeFB, ...etudeFB, ...priereFB, ...enseignementFB].map(v => v.videoId));
        const autresRaw = allFB.filter(v => {
            ...
            ].some(k => titleText.includes(k)) && !["prière", "priere", "31 jours"].some(k => text.includes(k));
        });
