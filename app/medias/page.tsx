      ...
      <Container className="py-9 pb-20 sm:py-9">
        ...
        <div className="mt-[34px] space-y-12">
          {active === ALL ? (
            ...
            <div className="grid grid-cols-1 gap-[26px] sm:grid-cols-2 lg:grid-cols-3">
              {items.map((item) => <MediaCardLarge key={item.videoId ?? item.title} item={item} />)}
            </div>
          )}
        </div>

        {active !== ALL && items.length === 0 ? (
        ...
        <Card className="mt-11 flex flex-col items-start gap-5 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7">
          ...
          <div className="flex gap-2.5">
            ...
            <a href="#" className="rounded-full bg-navy px-[18px] py-2.5 text-[12.5px] font-bold text-white hover:bg-navy-soft">TikTok</a>
          </div>
        </Card>
      </Container>
    </div>
